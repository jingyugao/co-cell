...
export default function App() {
  ...
  const { projects, refreshProjects, createProject, updateProject, upgradeSandbox, archiveSandbox, restoreSandbox } = useProjects();
  ...
  const environmentRestoring = Boolean(project?.sandboxUpgrade && project.sandboxUpgrade.phase !== 'failed' && ['preparing', 'restoring', 'verifying'].includes(project.sandboxUpgrade.phase));
  const environmentReclaimed = Boolean(project?.sandboxReclaimedAt && project.sandboxDataArchive && !project.sandbox);
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${sidebar ? 'open' : ''}`}>
      ...
      <nav className="session-list" aria-label="任务历史">{activeSessions.map(item => <button key={item.id} disabled={busy} className={`session-link ${page === 'chat' && selected === item.id ? 'active' : ''}`} onClick={() => selectSession(item.id)}><Icon name="chat" size={15} /><span>{item.title}</span>{item.status === 'running' && <span className="running-dot" />}</button>)}{!loading && !visibleSessions.length && <p className="empty-history">{project ? '此项目还没有会话。' : '创建项目，开始新的工作。'}</p>}{archivedSessions.length > 0 && <details className="archived-sessions"><summary>已归档 <span>{archivedSessions.length}</span></summary>{archivedSessions.map(item => <button key={item.id} disabled={busy} className={`session-link archived ${page === 'chat' && selected === item.id ? 'active' : ''}`} onClick={() => selectSession(item.id)}><Icon name="chat" size={15} /><span>{item.title}</span></button>)}</details>}</nav>
    ...
    {page === 'improvements' ? <ImprovementsPage onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} onOpenSession={openImprovementSession} /> : page === 'connections' ? <ConnectionsPage onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} /> : page === 'templates' ? <TemplatesPage onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} onDirtyChange={updateTemplatesDirty} onDefaultChanged={refreshConfig} /> : page === 'files' ? <SharedFilesPage onMenu={() => setSidebar(true)} onDirtyChange={updateFilesDirty} /> : page === 'projects' ? <ProjectsPage projects={projects} config={config} loading={loading} onRefresh={refreshProjects} onCreate={createProject} onUpdate={updateProject} onUpgradeSandbox={upgradeSandbox} onArchiveSandbox={archiveSandbox} onRestoreSandbox={restoreSandbox} onOpenProject={openProject} onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} /> : page === 'sandboxes' ? <SandboxManager onOpenProject={openProject} onOpenSession={selectSession} onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} /> : <main className="main-pane">
      <header className="topbar"><button className="icon-button mobile-only" aria-label="打开导航" onClick={() => setSidebar(true)}><Icon name="menu" /></button><div className="breadcrumbs"><span title={project?.name}>{project?.name || (settings ? basename(settings.workingDirectory) : 'Codex')}</span><span className="slash">/</span><strong>{session?.title || '新建会话'}</strong></div><div className="topbar-actions">{e2b && <span className="execution-badge" title="整个 Codex 在 E2B 沙箱内运行">E2B</span>}{session && <span className="connection-label"><span className={connected ? 'connection-dot' : 'offline-dot'} />{connected ? '已连接' : '重新连接中'}</span>}{session && <button className="session-archive-button" disabled={busy || running} title={isArchived(session) ? '移回项目会话列表' : '移入已归档会话列表'} onClick={() => void changeSessionArchived()}>{isArchived(session) ? '取消归档' : '归档会话'}</button>}<button className="raw-tools-toggle" aria-pressed={rawToolsOpen} onClick={() => { setRawToolsOpen(value => !value); setChangesOpen(false); }}><Icon name="code" size={15} />原始工具消息</button><button className={`icon-button ${changesOpen ? 'pressed' : ''}`} title="查看工作区改动" aria-label="查看工作区改动" aria-pressed={changesOpen} onClick={() => { setChangesOpen(value => !value); setRawToolsOpen(false); }}><Icon name="panel" /></button></div></header>
      ...
      {project && <div className="project-context"><strong>{project.name}</strong>{project.archivedAt && <span>已归档 · 仍可继续使用</span>}<span>共享项目工作区 · 独立会话上下文</span>{project.requirementUrl ? <><a href={project.requirementUrl} target="_blank" rel="noopener noreferrer">飞书需求 ↗</a>{project.requirementStatus && <span className="project-context-status">飞书项目：{project.requirementStatus}</span>}</> : <span>未关联飞书需求</span>}</div>}
      {project && environmentRestoring && <div className="project-run-note" role="status">正在创建新环境并恢复工作区和对话，完成后即可继续使用。</div>}
      {project && environmentReclaimed && !environmentRestoring && <div className="project-run-note" role="status">环境已回收，工作区和对话已备份。打开工作区或发送消息时会自动创建并恢复新环境。</div>}

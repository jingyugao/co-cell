...
export default function TurnItems({ turn, sessionId, onApprovalResolved, highlightedItemIds, ...resources }: {
  ...
  return <>
    {turn.items.map(item => {
      ...
      return <div key={item.id} id={`turn-${turn.id}-item-${item.id}`} className={highlightedItemIds?.includes(item.id) ? 'billing-item-selected' : undefined}>{approval ? <Fragment key={item.id}>{renderApproval(approval)}<ToolDetails item={item} /></Fragment>
        : <ItemView key={item.id} item={item} turnStatus={turn.status} timestamp={turn.itemTimestamps?.[item.id] || turn.startedAt} {...resources} />}</div>;
    })}

...
function ItemView({ item, turnStatus }: { item: ThreadItem; turnStatus: Turn['status'] }) {
  ...
  switch (item.type) {
    case 'agent_message': return <div className="agent-message"><Markdown text={item.text} /></div>;
    case 'reasoning': return <details className="reasoning"><summary><span className="tiny-orbit" />思考摘要<Icon name="chevron" size={12} /></summary><Markdown text={item.text} /></details>;
...
function TurnProgress({ turn, e2b = false }: { turn: Turn; e2b?: boolean }) {
  ...
  const label = turn.phase === 'starting' ? (e2b ? '正在准备 E2B 环境并启动 Codex…' : '正在启动 Codex…')
...
export default function App() {
  ...
  const projectBusy = Boolean(project?.activeSessionId && project.activeSessionId !== selected);
  ...
  useEffect(() => {
    ...
    Promise.all([api<AppConfig>('/api/config'), api<SessionSummary[]>('/api/sessions'), api<ProjectSummary[]>('/api/projects')]).then(([nextConfig, list, projectList]) => {
      ...
      const nextProject = current?.projectId ?? projectList.find(item => item.id === localStorage.getItem('codex-project'))?.id ?? projectList[0]?.id ?? null;
  ...
  function projectDeleted(id: string) {
    if (selectedProject === id) { setSelectedProject(null); setSelected(null); setSession(null); setPrompt(''); setAttachments([]); setDraftSettings(null); }
  }
  ...
  async function send() {
    if (!prompt.trim() || busy || running || localReadOnly || projectBusy || needsProject || !config || (selected && !session)) return;
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${sidebar ? 'open' : ''}`}>
      ...
      <div className="sidebar-label">当前项目</div><select className="project-picker" aria-label="当前项目" value={selectedProject || ''} onChange={event => { if (event.target.value) openProject(event.target.value); }}><option value="" disabled hidden>{projects.length ? '请选择项目' : '暂无项目'}</option>{projects.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button className={`sandbox-nav ${page === 'projects' ? 'active' : ''}`} aria-current={page === 'projects' ? 'page' : undefined} onClick={() => navigate('projects')}><Icon name="folder" />项目管理</button><button className="workspace-button" onClick={() => setSettingsOpen(true)} disabled={!settings || busy}><Icon name="folder" /><span>{settings ? basename(settings.workingDirectory) : '加载中…'}</span><Icon name="chevron" size={12} /></button>
    ...
    {page === 'connections' ? <ConnectionsPage onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} /> : page === 'templates' ? <TemplatesPage onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} onDirtyChange={updateTemplatesDirty} onDefaultChanged={refreshConfig} /> : page === 'files' ? <SharedFilesPage onMenu={() => setSidebar(true)} onDirtyChange={updateFilesDirty} /> : page === 'projects' ? <ProjectsPage projects={projects} config={config} loading={loading} onRefresh={refreshSessions} onOpenProject={openProject} onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} onDeleted={projectDeleted} /> : page === 'sandboxes' ? <SandboxManager onOpenProject={openProject} onOpenSession={selectSession} onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} /> : <main className="main-pane">
      ...
      {project && <div className="project-context"><strong>{project.name}</strong><span>共享项目工作区 · 独立会话上下文</span>{project.requirementUrl ? <a href={project.requirementUrl} target="_blank" rel="noopener noreferrer">飞书需求 ↗</a> : <span>未关联飞书需求</span>}</div>}
      ...
      {projectBusy && <div className="project-run-note" role="status">此项目的另一个会话正在执行任务，完成后可发送消息。<button onClick={() => selectSession(project!.activeSessionId)}>查看执行中的会话 ↗</button></div>}

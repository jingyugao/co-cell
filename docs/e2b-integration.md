...
| 任务运行中 | 每分钟将暂停 TTL 续为从现在起 24 小时；同项目多个会话共享工作区并行执行 |
| 一次使用结束 | 再续期 24 小时；读取工作区、打开服务预览也算使用 |
...
暂停 TTL 固定为 24 小时，旧的 `E2B_TIMEOUT_MS` 不再控制生命周期。E2B 单次连续运行硬上限仍独立存在：平台会将截止时间截断到「本次启动时间 + 团队 max_length_hours」，即使续期返回成功也不能突破。平台上限为 1 小时时，必须先提高平台限制，才能申请 24 小时 TTL。

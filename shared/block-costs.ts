...
export function sessionBlockCosts(turns: Turn[]) {
  const blocks = new Map<string, { id: string; label: string; cost: number; inputCost: number; outputCost: number; calls: number; turnId: string }>();
  ...
  for (const turn of turns) for (const call of turn.contextUsage ?? []) {
    ...
    for (const estimate of estimates) {
      ...
      const block = blocks.get(estimate.id) ?? { id: estimate.id, label: estimate.label, cost: 0, inputCost: 0, outputCost: 0, calls: 0, turnId: owner?.id ?? turn.id };

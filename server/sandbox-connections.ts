...
import { CONNECTION_ENVS, CONNECTION_ROOT, type ConnectionStore } from './connections.js';
...
export async function syncSandboxConnections(sandbox: Sandbox, store: ConnectionStore, signal: AbortSignal) {
  ...
  try {
    ...
    if ('KUBECONFIG' in envs) await sandbox.commands.run('mkdir -p /home/user/.kube/cache && chmod 700 /home/user/.kube', { user: 'user', signal, timeoutMs: 10_000 });
    ...
    throw new Error('沙箱命令工具或凭据同步失败，请检查工具安装和 E2B 连接');
  } finally {

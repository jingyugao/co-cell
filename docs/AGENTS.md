...
**当前重启限制：** Web 后端退出时会与已被 Sandbox App Server 接受的活动 turn 断开，启动后凭持久化的 `threadId` 和 `nativeTurnId` 重新观察同一个 turn，不会重复提交 prompt。重连期间通过 `thread/turns/list` 快照恢复当前可见内容，因此不能保证重放断线期间的每条瞬时通知；尚未被 App Server 接受或恢复身份不完整的任务仍会标记为中断。无感升级和步骤检查点尚未实现，更新运行服务前仍应尽量确认没有关键任务处于启动阶段。

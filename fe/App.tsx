...
export default function App() {
  ...
  const [archiveDetail, setArchiveDetail] = useState<{ key: string; meta: { sizeBytes: number; createdAt: string; sha256: string } | null } | null>(null);

  const [config, setConfig] = useState<AppConfig | null>(null);
  const refreshConfig = useCallback(() => { void api<AppConfig>('/api/config').then(setConfig).catch(() => {}); }, []);
  const [sessions, setSessions] = useState<SessionSummary[]>([]);
  const { projects, refreshProjects, createProject, updateProject, rebuildSandbox, backupProject } = useProjects();
  ...
  return <div className="app-shell">
    ...
    {archiveDetail && <ArchiveViewer archiveKey={archiveDetail.key} archiveMeta={archiveDetail.meta} onClose={() => setArchiveDetail(null)} />}

...
export default function RawToolMessages({ sessionId, onClose }: { sessionId: string | null; onClose: () => void }) {
  ...
  const [location, setLocation] = useState<'local' | 'e2b'>('local');
    ...
    <div className="raw-tools-content"><div className="raw-tools-source"><strong>{location === 'e2b' ? 'Codex E2B 会话记录' : 'Codex 本机会话记录'}</strong><p>展示 Agent 实际发出的工具调用及返回结果：包含 exec 输入代码、函数参数和输出。按 call_id 关联，独立于 SDK 的执行摘要。</p>{sandboxId && <p className="raw-tools-thread">E2B: <code>{sandboxId}</code></p>}{threadId && <p className="raw-tools-thread">Thread: <code>{threadId}</code></p>}<span>{messages.length} 条消息 · {groups.length} 组调用 / 输出{sessionId && ' · 每 1.5 秒刷新'}</span></div>
      ...
      {!sessionId ? <div className="raw-tools-empty">开始任务或选择已有会话后查看原始工具消息。</div>
        : loading && !messages.length ? <div className="raw-tools-empty" role="status"><span className="spinner" />正在读取原始工具消息…</div>
          : !messages.length && !error ? <div className="raw-tools-empty" role="status">{availability === 'missing' ? (location === 'e2b' ? '未找到此任务的 E2B 会话记录，沙箱或记录可能已不可用。' : '未找到此会话的本机记录。记录可能已移除，或来自其他设备。') : availability === 'pending' ? '正在等待 Codex 会话记录，生成后会自动显示。' : '此会话记录中暂时没有工具调用或工具输出。'}</div> : null}

...
import type { AppConfig, SandboxInventory, SandboxRecord } from '../../../protocol/types';
import { SandboxVersion } from '../../components/SandboxVersion';
import { api } from '../../lib/api';
import './SandboxManager.css';

const states = { running: '运行中', paused: '已暂停', unknown: '未知' };
...
function CleanupStatus({ sandbox }: { sandbox: SandboxRecord }) {
  ...
  return <small className="sandbox-cleanup-note unknown">未关联当前项目。确认无用后可手动删除。</small>;
}

export default function SandboxManager({ config, onOpenSession, onOpenProject, onMenu, onBack }: { config: AppConfig | null; onOpenProject: (id: string) => void; onOpenSession: (id: string) => void; onMenu: () => void; onBack: () => void }) {
    ...
    <div className="sandbox-scroll">
      ...
      {data?.enabled === false ? <div className="sandbox-empty"><h2>尚未配置 Docker Sandbox</h2><p>配置服务端 Docker后，即可查看沙箱和资源用量。</p></div> : <>
        <div className="sandbox-stats">
          <div><span>全部沙箱</span><strong>{data ? sandboxes.length : '—'}</strong><small>{data ? `${paused} 个已暂停 · ${dangling} 个悬挂` : '等待读取'}</small></div>

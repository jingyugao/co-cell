...
import { useSubagentConversations } from './features/chat/SubagentConversations';
...
import ArchiveViewer from './features/archives/ArchiveViewer';
...
function ConversationList({ items, archivedItems, selected, projectName, emptyMessage, busy, menuOpen, onMenuOpen, onSelect, onArchive }: {
  items: SessionSummary[];
  archivedItems?: SessionSummary[];
  ...
  const label = (item: SessionSummary) => projectName ? `${item.title} · ${projectName(item.projectId) ?? '未关联项目'}` : item.title;
  return <nav className="session-list" aria-label="对话列表">{items.map(item => <div key={item.id} className={`session-row ${selected === item.id ? 'active' : ''}`}><button disabled={busy} className="session-link" onClick={() => onSelect(item.id)} title={label(item)}><Icon name="chat" size={15} /><span>{label(item)}</span>{item.status === 'running' && <span className="running-dot" />}</button><div className="session-menu-wrap"><button disabled={busy} className="session-menu-btn" aria-label="会话操作" onClick={event => { event.stopPropagation(); onMenuOpen(menuOpen === item.id ? null : item.id); }}><svg width="12" height="12" viewBox="0 0 16 16"><circle cx="8" cy="3" r="1.5" fill="currentColor"/><circle cx="8" cy="8" r="1.5" fill="currentColor"/><circle cx="8" cy="13" r="1.5" fill="currentColor"/></svg></button>{menuOpen === item.id && <><button className="session-menu-backdrop" onClick={() => onMenuOpen(null)} /><div className="session-menu-dropdown"><button disabled={busy} className="session-menu-item" onClick={() => { onMenuOpen(null); onArchive(item.id); }}>归档会话</button></div></>}</div></div>)}{!items.length && <p className="empty-history">{emptyMessage}</p>}{archivedItems && archivedItems.length > 0 && <details className="archived-sessions"><summary>已归档 <span>{archivedItems.length}</span></summary>{archivedItems.map(item => <button key={item.id} disabled={busy} className={`session-link archived ${selected === item.id ? 'active' : ''}`} onClick={() => onSelect(item.id)}><Icon name="chat" size={15} /><span>{item.title}</span></button>)}</details>}</nav>;
}
...
export default function App() {
  ...
  const [archiveDetail, setArchiveDetail] = useState<{ key: string; versionId?: string;
    meta: { sizeBytes: number; bytesAdded?: number; createdAt: string } | null } | null>(null);

  const [config, setConfig] = useState<AppConfig | null>(null);
  const refreshConfig = useCallback(() => { void api<AppConfig>('/api/config').then(setConfig).catch(() => {}); }, []);
  const [sessions, setSessions] = useState<SessionSummary[]>([]);
  const { projects, refreshProjects, createProject, updateProject, rebuildSandbox, backupProject, switchSandbox } = useProjects();
  ...
  const [conversationScope, setConversationScope] = useState<'recent' | 'project'>('recent');
  ...
  const recentSessions = sessions.filter(item => !isArchived(item) && projects.some(project => project.id === item.projectId && (project.status ?? (project.archivedAt ? 'archived' : 'active')) === 'active')).sort((left, right) => right.updatedAt.localeCompare(left.updatedAt)).slice(0, 50);
  const projectName = (id: string | undefined) => {
    const owner = projects.find(item => item.id === id && !item.archivedAt);
    return owner ? projectDisplayName(owner) : undefined;
  };
  const systemManagement = page === 'sandboxes' || page === 'connections' || page === 'files' || page === 'improvements';
  ...
  const conversationTurns = session?.turns ?? [];
  ...
  useLayoutEffect(() => {
    ...
    area.scrollTop = area.scrollHeight - previous.height + previous.top;
  ...
  async function saveSettings(next: Settings, threadId: string, title: string) {
    if (threadId.trim()) { const created = await api<Session>('/api/sessions', { method: 'POST', body: JSON.stringify({ ...(project ? { projectId: project.id } : {}), settings: next, threadId: threadId.trim(), title: title || '导入的 CoCell 会话' }) }); selectSession(created.id); setSession(created); }
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${sidebar ? 'open' : ''}`}>
      <div className="brand"><Mark small /><strong>CoCell</strong><span className="demo-tag">WEB</span><button className="icon-button mobile-only" onClick={() => setSidebar(false)} aria-label="关闭导航"><Icon name="close" /></button></div>
      <div className="workspace-switcher" role="tablist" aria-label="工作区域"><button role="tab" aria-selected={!systemManagement} className={!systemManagement ? 'selected' : ''} onClick={() => navigate('chat')}>工作台</button><button role="tab" aria-selected={systemManagement} className={systemManagement ? 'selected' : ''} onClick={() => navigate('sandboxes')}>系统管理</button></div>
      {!systemManagement ? <><button className="new-session" disabled={busy} onClick={() => project ? selectSession(null) : navigate('projects')}><Icon name="plus" />新建会话<span>↗</span></button><div className="sidebar-label">当前项目</div><ProjectPicker projects={projects} selected={selectedProject} onSelect={openProject} /><button className={`sandbox-nav ${page === 'projects' ? 'active' : ''}`} aria-current={page === 'projects' ? 'page' : undefined} onClick={() => navigate('projects')}><Icon name="folder" />项目管理</button><div className="conversation-tabs" role="tablist" aria-label="对话范围"><button role="tab" aria-selected={conversationScope === 'recent'} className={conversationScope === 'recent' ? 'selected' : ''} onClick={() => setConversationScope('recent')}>最近对话</button><button role="tab" aria-selected={conversationScope === 'project'} className={conversationScope === 'project' ? 'selected' : ''} onClick={() => setConversationScope('project')}>项目会话</button></div><ConversationList items={conversationScope === 'recent' ? recentSessions : activeSessions} archivedItems={conversationScope === 'project' ? archivedSessions : undefined} selected={selected} projectName={conversationScope === 'recent' ? projectName : undefined} emptyMessage={conversationScope === 'recent' ? '还没有最近对话。' : project ? '此项目还没有会话。' : '请先选择项目。'} busy={busy} menuOpen={sessionMenuOpen} onMenuOpen={setSessionMenuOpen} onSelect={selectSession} onArchive={id => void archiveSessionById(id)} /></> : <><div className="sidebar-label">系统管理</div><button className={`sandbox-nav ${page === 'sandboxes' ? 'active' : ''}`} aria-current={page === 'sandboxes' ? 'page' : undefined} onClick={() => navigate('sandboxes')}><Icon name="panel" />沙箱管理</button><button className={`sandbox-nav ${page === 'connections' ? 'active' : ''}`} aria-current={page === 'connections' ? 'page' : undefined} onClick={() => navigate('connections')}><Icon name="globe" />连接与凭据</button><button className={`sandbox-nav ${page === 'files' ? 'active' : ''}`} aria-current={page === 'files' ? 'page' : undefined} onClick={() => navigate('files')}><Icon name="code" />共享文件</button><button className={`sandbox-nav ${page === 'improvements' ? 'active' : ''}`} aria-current={page === 'improvements' ? 'page' : undefined} onClick={() => navigate('improvements')}><Icon name="chat" />改进建议</button></>}
      <div className="sidebar-footer"><div className="runtime-label"><span className={config ? 'connection-dot' : 'offline-dot'} />{config ? `CoCell Agent · ${config.codexVersion}` : '正在连接服务'}<span>TS</span></div></div>
    </aside>
    {page === 'improvements' ? <ImprovementsPage onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} onOpenSession={openImprovementSession} /> : page === 'connections' ? <ConnectionsPage onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} /> : page === 'files' ? <SharedFilesPage onMenu={() => setSidebar(true)} onDirtyChange={updateFilesDirty} /> : page === 'projects' ? <ProjectsPage projects={projects} config={config} loading={loading} onRefresh={refreshProjects} onCreate={createProject} onUpdate={updateProject} onRebuildSandbox={rebuildSandbox} onBackup={backupProject} onSwitchSandbox={switchSandbox} onOpenProject={id => openProject(id, true)} onViewBackup={(key, versionId, meta) => setArchiveDetail({ key, versionId, meta })} onMenu={() => setSidebar(true)} onBack={returnToChat} /> : page === 'sandboxes' ? <SandboxManager config={config} onOpenProject={openProject} onOpenSession={selectSession} onMenu={() => setSidebar(true)} onBack={() => navigate('chat')} /> : <main className='main-pane'>
	      <header className='topbar'><button className='icon-button mobile-only' aria-label='打开导航' onClick={() => setSidebar(true)}><Icon name='menu' /></button><div className='breadcrumbs'><span title={project ? projectDisplayName(project) : undefined}>{project ? projectDisplayName(project) : (settings ? basename(settings.workingDirectory) : 'CoCell')}</span><span className='slash'>/</span><strong>{session?.title || '新建会话'}</strong></div><div className='topbar-actions'><button className={'icon-button ' + (showBilling ? 'pressed' : '')} aria-label='费用估算' disabled={!session} onClick={() => setShowBilling(!showBilling)}><svg width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'><path d='M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6'/></svg></button><div className='notif-bell-wrapper'><button className='icon-button notif-bell' aria-label='消息通知' onClick={() => { setNotifOpen(!notifOpen); if (notifDot && notifications.length > 0) { localStorage.setItem('codex-last-notif', notifications[0].id); setNotifDot(false); } }}>{notifDot && <span className='notif-dot' />}<svg width='18' height='18' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'><path d='M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9'/><path d='M10.3 21a1.94 1.94 0 0 0 3.4 0'/></svg></button>{notifOpen && <><button className='session-menu-backdrop' onClick={() => setNotifOpen(false)} /><div className='notif-dropdown'><div className='notif-dropdown-header'>消息通知</div><div className='notif-dropdown-body'>{notifications.length === 0 ? <p className='notif-empty'>暂无消息</p> : notifications.slice(0, 20).map(n => <button key={n.id} className='notif-item' onClick={() => { setNotifOpen(false); selectSession(n.sessionId); }}><span className={'notif-type-badge ' + (n.type.startsWith('approval') ? 'approval' : 'turn')}>{n.type.startsWith('approval') ? '审批' : n.type === 'turn_completed' ? '完成' : n.type === 'turn_failed' ? '失败' : '停止'}</span><span className='notif-title'>{n.title}</span><span className='notif-body'>{n.body}</span></button>)}</div></div></>}</div></div></header>
      ...
      {session?.historyError && <div className="error-banner" role="status">{session.historyError}</div>}
      {subagentsError && <div className="error-banner" role="status">子代理对话暂时无法加载，请刷新页面重试。</div>}
      ...
      <div className="content-layout">{session && showBilling && <BillingRail sessionId={session.id} turns={conversationTurns} selectedBlockId={billingSelection?.blockId} onSelectBlock={(blockId, turnId, itemIds) => { const targetId = blockId.endsWith(':input') ? itemIds?.at(-1) : itemIds?.[0]; setBillingSelection({ blockId, turnId, itemIds: targetId ? [targetId] : [] }); followOutput.current = false; document.getElementById(`turn-${turnId}-item-${targetId}`)?.scrollIntoView({ block: 'start', behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' }); }} />}<section className="conversation">

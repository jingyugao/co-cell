...
export default function TurnContextStatus({ turn }: { turn: Turn }) {
  ...
  const text = turn.codexAccepted === false
    ? turn.status === 'running' ? '等待 CoCell 接收' : '未收到 CoCell 接收回执'
    : '历史记录 · 未确认 CoCell 接收状态';
  return <span className="turn-context-status" title="平台保存的是提交记录；是否进入上下文以 CoCell 原生记录为准。">{text}</span>;
}

...
export interface DeveloperReadonlyRule {
  apiGroups: string[];
  resources: string[];
  verbs: string[];
}
...
export const DEVELOPER_READONLY_RULES: readonly FrozenRule[] = Object.freeze(matrix.map(([group, resources]) => Object.freeze({
...
const expectedPermissions = new Set(DEVELOPER_READONLY_RULES.flatMap(rule => rule.apiGroups.flatMap(group => rule.resources.flatMap(resource => rule.verbs.map(verb => permission(group, resource, verb))))));
 ...
 */
export function validateDeveloperReadonlyRules(value: unknown): boolean {
  try {
...
export type DeveloperReadonlyManifest =
  ...
  | { apiVersion: 'rbac.authorization.k8s.io/v1'; kind: 'ClusterRole'; metadata: { name: string }; rules: DeveloperReadonlyRule[] }
...
export function createDeveloperReadonlyManifests(): DeveloperReadonlyManifest[] {
  return [
    ...
    { apiVersion: 'rbac.authorization.k8s.io/v1', kind: 'ClusterRole', metadata: { name: KUBERNETES_DEVELOPER_CLUSTER_ROLE },
      rules: DEVELOPER_READONLY_RULES.map(rule => ({ apiGroups: [...rule.apiGroups], resources: [...rule.resources], verbs: [...rule.verbs] })) },

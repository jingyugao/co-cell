...
const labels: Record<View, string> = { input: '输入参数', output: '输出结果', raw: 'SDK JSON' };
...
function inspect(item: ThreadItem): { input?: unknown; output?: unknown; note?: string } {
  switch (item.type) {
    ...
    case 'command_execution': return {
      ...
      note: 'SDK 提供的是执行命令，未包含原始工具调用的全部参数。',
    };
    case 'file_change': return {
      output: { changes: item.changes, status: item.status },
      note: '这里只是 SDK 文件变更摘要。实际 apply_patch 补丁请在顶部「原始工具消息」中查看。',
    };
    case 'web_search': return { input: { query: item.query }, note: 'SDK 提供搜索词，未提供搜索结果正文。' };
    default: {
      const raw = item as unknown as Record<string, unknown>;
      return { input: raw.arguments, output: raw.result, note: '完整 JSON 保留 SDK 返回的全部 item 字段。' };
    }
  }
}
...
export default function ToolDetails({ item }: { item: ThreadItem }) {
  ...
  return <details className="tool-inspector" onToggle={event => { setOpen(event.currentTarget.open); setCopyStatus(''); }}>
    ...
    {open && <div className="tool-inspector-body">
      ...
      <div className="tool-inspector-note">{view === 'raw' ? '这是 SDK item 快照。Agent 的实际工具调用和返回结果请查看顶部「原始工具消息」。' : fields.note}</div>
      {json === undefined ? <p className="tool-inspector-empty">SDK 未提供{labels[view]}字段。可在顶部「原始工具消息」查看实际调用记录。</p> : <pre aria-label={labels[view]}><code>{json}</code></pre>}
